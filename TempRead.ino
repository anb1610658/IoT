...
#define DHTPIN 2            // Chân dữ liệu của DHT11 kết nối với GPIO4 của ESP8266
#define DHTTYPE DHT11       // Loại DHT được sử dụng

DHT dht(DHTPIN, DHTTYPE);
WiFiClient client;          // Tạo 1 biến client thuộc kiểu WiFiClient
const char* ssid = "sony";      // Tên mạng Wifi được chỉ định sẽ kết nối (SSID)
const char* password = "cobietdau";  // Password của mạng Wifi được chỉ định sẽ kết nối
const char* server = "192.168.43.43";     // Địa chỉ IP của máy khi truy cập cùng mạng WiFi
